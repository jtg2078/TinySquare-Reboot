...
-(void)setType:(MKInfoPanelType)type {
    if(type == MKInfoPanelTypeError) {
        ...
        self.backgroundGradient.image = [[UIImage imageNamed:@"Black"] stretchableImageWithLeftCapWidth:1 topCapHeight:5];
	...
	else if(type == MKInfoPanelTypeProgress) {
		self.backgroundGradient.image = [[UIImage imageNamed:@"Black"] stretchableImageWithLeftCapWidth:1 topCapHeight:5];
        self.titleLabel.font = [UIFont boldSystemFontOfSize:15];
...
- (void)hidePanelAfter:(float)seconds {
	[self performSelector:@selector(removeFromSuperview) withObject:nil afterDelay:seconds];
}
