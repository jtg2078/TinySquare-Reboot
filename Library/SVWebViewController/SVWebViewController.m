...
- (void)viewDidLoad {
	...
	UIButton* backButton1 = [self.navigationController setUpCustomizeBackButtonWithText:NSLocalizedString(@"返回", nil)];
	self.navigationItem.leftBarButtonItem = [[[UIBarButtonItem alloc] initWithCustomView:backButton1] autorelease];
	...
	if(!deviceIsTablet) {
		...
		if(self.navigationController == nil) {
			...
			label.text = NSLocalizedString(@"載入中, 請稍後...", nil);
			label.numberOfLines = 2;
			label.textColor = [UIColor blackColor];
			label.textAlignment = UITextAlignmentCenter;
			label.backgroundColor = [UIColor clearColor];
			label.frame = CGRectMake(0, 0, 230, 44);
			label.font = font;
...
- (void)showActions {
	...
	UIActionSheet *actionSheet = [[UIActionSheet alloc] 
						  ...
						  otherButtonTitles: NSLocalizedString(@"在 Safari 打開", @"SVWebViewController"), nil]; 
	
	
	if([MFMailComposeViewController canSendMail])
		[actionSheet addButtonWithTitle:NSLocalizedString(@"郵寄此網頁的連結", @"SVWebViewController")];
	
	[actionSheet addButtonWithTitle:NSLocalizedString(@"取消",@"SVWebViewController")];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
	...
	if([[actionSheet buttonTitleAtIndex:buttonIndex] isEqualToString:NSLocalizedString(@"在 Safari 打開", @"SVWebViewController")])
		[[UIApplication sharedApplication] openURL:self.webView.request.URL];
	
	else if([[actionSheet buttonTitleAtIndex:buttonIndex] isEqualToString:NSLocalizedString(@"郵寄此網頁的連結", @"SVWebViewController")]) {
