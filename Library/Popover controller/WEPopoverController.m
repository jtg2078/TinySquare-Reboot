...
- (void)presentPopoverFromRect:(CGRect)rect 
	...
	if(contentViewController.view == nil)
		NSLog(@"bad bad bad! contentViewController.view is not suppose to be nil");
	...
	backgroundView = [[WETouchableView alloc] initWithFrame: [[UIScreen mainScreen] applicationFrame]];
	CGRect adjustedToCoverNaviBar = backgroundView.frame;
	adjustedToCoverNaviBar.origin.y = -44;
	adjustedToCoverNaviBar.size.height = 504; // which covers the navigation bar as well
	backgroundView.frame = adjustedToCoverNaviBar;
...
- (UIView *)keyView {
	...
	if (w.subviews.count > 0) {
		return [w.subviews objectAtIndex:1]; // should be zero in normal case, but since my background image view is at 0
	} else {
		return w;
	}
}
